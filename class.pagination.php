...
class Pagination 
{
	...
	function create_links()
	{
		...
		if ($currentPage > 1){
			$output .= sprintf('<li><a href="' . $link . '">prev</a></li>', $currentPage - 1);
		}else{
            $output .= sprintf('<li>prev</li>', $totalPages);
		}
		if ($currentPage<1)
            $currentPage=1;
            
	   $output .= "<li style='padding-left:10px;padding-right:10px;'>".  $currentPage ."/".$totalPages ."</li>";

		if ($currentPage < $totalPages){
			$output .= sprintf('<li><a href="' . $link . '">next</a></li>', $currentPage + 1);
		}else{
		  	$output .= sprintf('<li>next</li>', 1);
		}
