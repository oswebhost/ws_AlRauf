...
$per_page=10;
$q = mysql_query("select * from news where news='1' order by rid desc ") or die(mysql_error());
...
$q = mysql_query("SELECT * from news  where news='1' order by rank, rid desc LIMIT $PAGE,$per_page") or die (mysql_error());
...
if (!isset($_GET[ID])) :
	...
	while ($d = mysql_fetch_array($q) ):
		$rowcol=rowcol($n++);
		$page_content .= "<tr >\n";
		$page_content .= "<td valign='top'  width='20'><img src='images/news-icon.gif' border='0'></td>\n";
		$page_content .= "<td valign='top' style='padding-bottom:10px;' $rowcol ><div id='link'><a href='$PHP_SELF?ID=$d[rid]&PAGE=$p'>$d[title]</a></div>\n\n";
		$page_content .= "<p style='padding:0;'>" . stripslashes(substr(strip_tags($d["content"]),0,560)) . "</td>";
		$page_content .= "</tr>";
	...
	$ch = "\n\n" ;
	$q = mysql_query("select * from news where rid='$_GET[ID]'") or die(mysql_error());
	$d = mysql_fetch_array($q) ;
	
	$cap = trim($d["title"]) ;
	
	$page_content .= '<P><font size="+1" color="#00000">' . trim($d["title"])  .'</font><br>' . $ch ;
	$page_content .= "<i>".  $d['post_date'] . "</i>" ;

	$page_content .=  trim(stripslashes($d["content"])) . $ch;

	$page_content .=  '<div style="padding-left:10px; padding-bottom:20px;" id="link"><a href="'. $PHP_SELF .'?PAGE='. $p .'"> лл go back</a></div>';
endif;	


$PAGE_TITILE = "$cap News or Articles " . $PAGE_TITILE ;
...
$p = $_GET["PAGE"];
echo  "<h3 style='margin-bottom:10px;'>News </h3>";
