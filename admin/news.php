...
$pageNav=""; $heading = "News" ; $tbname = "news" ; $ORDER="rank, title" ;

 if ($_REQUEST['GO']=="SAVE"){
      
    $d_answer = addslashes(trim($_POST['answer'])) ;
    $title    = addslashes(trim($_POST['title'])) ;
    $post_date= addslashes(trim($_POST['post_date'])) ;
    $rank     = (int) $_POST['rank'];
    $ID       = $_REQUEST['ID'];
    
    if ( ($_REQUEST['ACTION']=='ADD')):
        $q = mysql_query("insert into $tbname (title, rank, content,news,post_date) VALUES ('$title','$rank', '$d_answer', '1','$post_date') ") or die ( mysql_error() ) ;
        $ID = mysql_insert_id() ; 
    ...
    if ( ($_REQUEST['ACTION']=='EDIT') ):
         $q = mysql_query("UPDATE $tbname set title='$title', rank='$rank', content='$d_answer', post_date='$post_date' where rid='$ID'") or die ( mysql_error() ) ;
    endif;
 } 
...
$query="SELECT * from $tbname where news='1' order by $ORDER LIMIT $PAGE,$per_page";
$result= mysql_query($query) or die( mysql_error() );
$list="";
$number=0;
